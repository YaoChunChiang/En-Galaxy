...
try{
  ...
  if( isset($_POST["dataInfo"])===true){//寫入資料庫
   ...
   elseif (isset($_REQUEST['ansReport'])===true) {
    ...
    $sql="UPDATE activity m left join activity_report r on m.act_no=r.act_no 
          set m.act_status= :reportStatus 
          where r.act_repono = :repoNo";
     $reportAnswer=$pdo->prepare($sql);
    ...
    $sql="UPDATE member_question m left join question_report r on m.que_no=r.que_no 
          set m.que_status= :reportStatus 
          where r.que_repono = :repoNo";
     $reportAnswer=$pdo->prepare($sql);
    ...
    echo "問題檢舉下架成功<br>";
      ...
      $sql="DELETE FROM question_report 
            WHERE que_repono= :queRepoNo";
       $reportAnswer=$pdo->prepare($sql);
     ...
     elseif (isset($_REQUEST['ansRepoNo'])===true) {
      $sql="DELETE FROM answer_report 
            WHERE answer_report= :ansRepoNo";
       $reportAnswer=$pdo->prepare($sql);
     ...
     elseif (isset($_REQUEST['actRepoNo'])===true) {
      $sql="DELETE FROM activity_report 
            WHERE act_repono= :actRepoNo";
       $reportAnswer=$pdo->prepare($sql);
    ...
    $chooseBest->execute();
    // 更新問題的最佳答案
    $sqlQuestionBest="update member_question q 
                      SET ans_no=(SELECT ans_no from member_answer a where q.que_no=a.que_no and best_ans =1)";
    $QuestionBest=$pdo->prepare($sqlQuestionBest);
    $QuestionBest->execute();
    //找到回答問題的會員給獎金
    $sqlBounty=" UPDATE mem_main 
                 set mem_money= mem_money  + :que_money 
                 WHERE mem_no=( SELECT a.mem_no from (SELECT * from mem_main) m left join member_answer a on m.mem_no=a.mem_no left join member_question q on a.que_no=q.que_no where a.que_no = :que_no and q.ans_no=a.ans_no)";
    $Bounty=$pdo->prepare($sqlBounty);
    $Bounty->bindValue(":que_money",$_REQUEST['que_money']);
    $Bounty->bindValue(":que_no",$_REQUEST['queNo']);
    $Bounty->execute();
    //回傳資訊至前台告知誰獲得了獎金
    $sqlMember="select * from mem_main
                where mem_no=( SELECT a.mem_no from (SELECT * from mem_main) m left join member_answer a on m.mem_no=a.mem_no left join member_question q on a.que_no=q.que_no where a.que_no = :que_no and q.ans_no=a.ans_no)";
