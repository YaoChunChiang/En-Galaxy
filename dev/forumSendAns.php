...
try{
  ...
  if( isset($_REQUEST['que_no'])===true){//寫入資料庫
   ...
   elseif (isset($_REQUEST['ansReport'])===true) {
    ...
    $sno=$_GET['no'];
    $sql="select m.mem_name, a.ans_desc,a.time, q.que_no from member_question q left join member_answer a on q.que_no = a.que_no left join mem_main m on a.mem_no =m.mem_no where q.que_no ={$sno} and a.ans_status=1";
