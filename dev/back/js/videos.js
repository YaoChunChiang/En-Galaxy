...
/*按刪除鍵 studyEngMag.php 開始*/
    function videoDel(e){
      ...
      $.post('studyEngAdd.php',{videoNum, who: 'deleteVideo'},
          function(data){
            console.log(data)
          }
      );
...
/*按修改鍵轉成input studyEngMag.php 結束*/
    ...
    function edit(){
      // alert();
        ...
        $(this).parents('tr').find('input').attr('readonly', false).attr('disabled', false).css('border', '1px solid #ccc');
        $(this).siblings().removeClass('d-none');

    }

    function editConfirm(){
        let videoNo = $(this).parents('tr').find('.videoNo').text();
        let levelNo = $(this).parents('tr').find('input.level_no').val();
        let videoName = $(this).parents('tr').find('input.video_name').val();
        let videoDesc = $(this).parents('tr').find('input.video_desc').val();
        let videoType = $(this).parents('tr').find('input.video_type').val();
        let videoStatus = $(this).parents('tr').find('input:checkbox').val();
        // console.log(levelNo, videoName, videoDesc, videoType,videoStatus);
        
        $.get('studyEngAdd.php', {who: 'editVideo', levelNo, videoName, videoDesc, videoType, videoStatus, videoNo}, (eee)=>{
            if(eee === 'weeee'){
                alert('異動成功');
                location.reload();
            }
    ...
    let checkboxs = document.getElementsByName('video_q_status');
    checkboxs.forEach(checkbox => {
        checkbox.addEventListener('change', changeVideoStatus);
    })

    $('.questionModifyConfirm').click(editConfirm);
    $(".edit").click(edit);
